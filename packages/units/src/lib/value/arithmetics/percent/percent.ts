...
declare module '@table-q/units' {
  interface Value<Signed, U, Decimals> {
    percent(value: Numeric | Scalar): Value<Signed, U, Decimals>;
  }
}

export function percent<Signed extends SignedParam, U extends UNIT, Decimals extends number>(
  ...
  retValue.denominator *= denominator * 100n;
  return cleanup(ret);
}

export function extend(proto: Value) {
  proto.percent = function (value) {
    return percent.apply(this, [value]);
  };
}
